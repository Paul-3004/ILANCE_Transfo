    ...
    if version == 1 or version == 2:
        ...
        '''
            ...
            tgt_embedder = Embedder(nlayers = config["nlayers_embedder_tgt"], d_input= config["d_input_decoder"], 
                                    d_model = config["d_out_embedder_tgt"], d_hid= config["d_ff_embedder_tgt"])
            tracks_embedder = Embedder(nlayers = config["nlayers_embedder_tracks"], d_input= config["d_input_tracks"], 
                                       d_model = config["d_out_embedder_tracks"], d_hid= config["d_ff_embedder_tracks"])
            
            decoder_feats_layer = TransformerDecoderLayer(d_model = config["d_out_embedder_src"], nhead = config["nhead"],
            ...
            if version == 3:
                decoder_labels_layer = TransformerDecoderLayer(d_model = config["d_out_embedder_tgt"], nhead = config["nhead"],
                                                               ...
                                                               norm_first= config["norm_first"], bias= config["bias"],
                                                               device = DEVICE, dtype = dtype)
                decoder_labels = TransformerDecoder(decoder_labels_layer, config["nlayers_decoder"])
            else:
                decoder_labels_layer = CustomDecoderLayer(d_label = config["d_out_embedder_tgt"], 
                                               d_proj = config["d_out_embedder_src"],
                                               nproj = config["nproj"],
                                               nhead = config["nhead"],
                                               dim_ff_sub = config["dim_ff_sub_decoder"],
