...
class CollectionHits(Dataset):
    ...
    def formatting(self, feats, labels, special_symbols):
        ...
        E = np.sqrt(np.square(labels[...,1]) + pvec_norm2) # E = sqrt{m^2 + p^2}
...
def get_data(dir_path, batch_size, special_symbols):
    data_set = CollectionHits(dir_path,special_symbols)          
...
data_ld = get_data("data",25,special_symbols)
